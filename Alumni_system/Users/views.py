...
import json
import requests
from .models import Alumni_User, Permanent_User
...
def activate(request, uidb64, token):
    ...
    except(TypeError, ValueError, OverflowError, user12.DoesNotExist):
        user12 = None
...
def validated_login(email, password, verify):
    user = Permanent_User.objects.filter(email = email)
    for i in user:
        if i.password == password and verify:
            return True
...
def login(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            result = validated_login(email, password, verify)
            print(result)
...
def forget_password(request):
    ...
    if request.method == 'POST':
        ...
        try :
            ...
            email.send()
            return redirect('login')
        except(TypeError, ValueError,user12.DoesNotExist):
